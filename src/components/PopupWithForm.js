  ...
  return (
    ...
    <div className={`popup popup-${props.name} ${props.isOpen ? 'popup_opened' : ''}`} >
      <div className={`popup-${props.name}__container popup__container`}>
        ...
        <form className={`popup-${props.name}__form popup__form`} name={props.name} noValidate>
        {props.children}
          <button className={`popup-${props.name}__button popup__button`} type="submit" disabled>Сохранить</button>
        </form>
      </div>
    </div>
